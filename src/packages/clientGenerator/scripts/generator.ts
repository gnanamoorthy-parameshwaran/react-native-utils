...
import type {Operation, OpenAPI, Parameter, PathItem, Reference, Schema} from '../types/OpenAPISpec.ts';
...
class APIClientGenerator {
    ...
    public generate() {
        ...
        Object.keys(schemas).forEach(key => this.schemaGroups.set(key, schemaGroupName(key)));
    ...
    private resolveOperation(uri: string, method: HttpMethodKey, operation: Operation): ResolvedOperation {
        const {version, folder, resource, methodName} = this.resolveNaming(uri, method, operation);
     ...
     */
    private resolveCache(method: HttpMethodKey, operation: Operation, queryParams: ResolvedQueryParam[]): ResolvedCache | undefined {
    ...
    private resolveRequestBody(operation: Operation, methodPascal: string): ResolvedRequestBody | undefined {
    ...
    private writeTypeFiles(operations: ResolvedOperation[], schemas: Record<string, Schema>) {
        ...
        groups.forEach((group, groupName) => this.writeTypeFile(groupName, group, schemas));
    }
    ...
    private writeTypeFile(groupName: string, group: SchemaGroup, schemas: Record<string, Schema>) {
     ...
     */
    private buildObjectParam(fields: {name: string; type: string; required: boolean}[]): string {
    ...
    private buildMethod(operation: ResolvedOperation) {
        ...
        if (operation.requestBody && operation.requestBody.type !== 'FormData') {
            ...
            const bodyGroup = operation.requestBody.inlineType ? operation.resource : this.schemaGroups.get(bodyType);
