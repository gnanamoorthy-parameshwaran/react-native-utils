...
export default function createAPI({baseURL, throwException, cache = new MemoryCacheDriver(), onBeforeRequest, onAfterResponse}: CreateAPIProps) {
        ...
        const request = async <T>(options: FetchProps) => {
            ...
            try {
                if (options.cacheConfig) {
                    const cacheKey = options.cacheConfig.key ?? getCacheKey({method, endpoint: options.endpoint});
